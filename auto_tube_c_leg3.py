...
async def generate_image(prompt, filename, negative_prompt="", data_dir="data"):
    ...
    try:
        ...
        quality_negative = """shirtless, nsfw, naked, deformed, strabismus, duplicated, bad anatomy, bad proportions, disfigured, gross proportions, cloned, black and white"""
        
        # enhanced_negative = f"{base_negative}, {safety_negative}, {anatomical_negative}, {quality_negative}"
        # enhanced_negative = f"{quality_negative}, {base_negative}"
        ...
        #     "width": int(round(v_width * quality)),
        #     "height": int(round(v_height * quality)),
                    ...
                    "args": [
                        {
                            ...
                            "ad_steps": 60,
                            "ad_cfg_scale": 6,
                            "ad_negative_prompt": enhanced_negative
                        }
                    ]
                }
            }
...
async def text_to_speech_edge(text, filename):
    ...
    VOICE = VOICES[0]
    communicate = edge_tts.Communicate(text, VOICE, pitch="-4Hz", rate="+12%")
...
def convert_whisper_segments(segments: List[Dict], words_per_segment: int = 12, min_silence: float = 0.3) -> List[tuple]:
    """
