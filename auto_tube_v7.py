...
def parse_arguments():
    ...
    parser.add_argument('--script_mode', type=str, default='manual', help='modo do script:"manual" ou "temp_f" ')
    ...
    parser.add_argument('--duration', type=int, default=60, help='Duração desejada do vídeo em segundos (default: 60)')
    parser.add_argument('--sd_steps', type=int, default=40, help='Número de steps para o Stable Diffusion (default: 40)')
    parser.add_argument('--v_width', type=int, default=1280, help='largura do vídeo (default: 1280)')
    parser.add_argument('--v_height', type=int, default=1280, help='Altura do vídeo (default: 1280)')
    parser.add_argument('--v_fps', type=int, default=30, help='FPS do vídeo (default: 30)')
    parser.add_argument('--paymode', type=str, default='default', help='modo payload SD (movie, shorts ou default)')
    ...
    parser.add_argument('--voice_speed', type=float, default=1.0, help='velocidade da voz de narração (default: 1.0)')
...
async def generate_image(prompt, filename, negative_prompt="", data_dir="data"):
    ...
    try:
        ...
        quality_style = (
            "comic book illustration of"
        )
        
        art_style = (
            "illustration, soft pastel colors, whimsical and natural lighting"
        )
        
        
        enhanced_prompt = f"{prompt}"
        
        quality_negative = """shirtless, nsfw, naked, duplicated, twin, bad anatomy, bad proportions, disfigured, gross proportions, cloned, black and white, multiple people, twins, two persons, duplicate person, second character, repeating figures, clone characters"""
        ...
        if args.paymode == 'movie':
            payload = {
                ...
                "refiner_switch": 0.4,  # Momento de troca para o refinador
                "override_settings": {
        ...
        elif args.paymode == 'shorts':
            payload = {
                "prompt": f"comic book illustration of, {prompt}",
                "negative_prompt": enhanced_negative,
                "steps": stable_difusion_steps,
                "width": 360,
                "height": 640,
                ...
                "karras": True,
                "refiner_checkpoint": "realisticComicBook_v10",  # Modelo do refinador
                "refiner_switch": 0.5,  # Momento de troca para o refinador
                "override_settings": {
                    "sd_model_checkpoint": "wildlifexAnimals_wildlifexAnimals"  # Modelo principal
                },
                "override_settings_restore_afterwards": True,
                "enable_hr": True,              # Ativa o hires.fix
                "hr_upscaler": "Latent",       # Tipo de upscaler
...
def image_to_video_clip(image_path, output_video, duration):
    ...
    """
    # print("FFMPEG_BINARY:", get_setting("FFMPEG_BINARY"))
    # print("IMAGEMAGICK_BINARY:", get_setting("IMAGEMAGICK_BINARY"))
    try:
        # Carregar imagem
...
async def generate_script(prompt, path, model='gemma2:9b', duration=120):
    ...
    try:
        ...
        system_prompt = f"""
Você é um assistente especializado em criar roteiros para vídeos criativos e filosóficos. 
...
- Nos prompts de imagem, inclua detalhes sobre o ambiente, estilo visual, iluminação e elementos adicionais que ajudem a criar uma cena mais rica e imersiva.
- O texto a para narração precisa ser em português. Exemplo: "Enquanto ajustava o telescópio, João notou um padrão incomum de luz chamou sua atenção. As estrelas pareciam piscar em uma sequência rítmica, como se tentassem comunicar algo."
...
{{
    "scenes": [
        {{
            "image_prompt": "Wide shot of Norman Bates standing in front of an isolated motel on a rainy night, with shadows and a dimly lit neon sign.",
            "narration": "Norman Bates é mais que um nome; é um símbolo de como o trauma pode moldar a mente humana. Tudo começa com seu isolado motel, uma fachada tranquila para segredos sombrios.",
            "duration": 10
        }},
        {{
            "image_prompt": "Interior view of Norman Bates sitting alone at a table in an old, dimly lit house, lost in thought.",
            "narration": "Desde cedo, Norman cresceu sob a influência sufocante de sua mãe, Norma. Uma relação cheia de controle e manipulação, que plantou as sementes de sua complexa psique.",
            "duration": 10
        }},
        {{
            "image_prompt": "A young boy walking through a field, with a large, imposing house in the background.",
            "narration": "Após a morte de seu pai, Norman enfrentou um novo tipo de solidão. Sua mãe tornou-se sua única companhia, moldando sua visão de mundo e suas relações.",
            "duration": 10
        }}
    ],
    "seo": {{
        "title": "Norman Bates - A Mente Por Trás do Horror",
        "description": "Explore os aspectos psicológicos de Norman Bates, a inspiração por trás de 'Psycho', e como traumas moldaram um dos personagens mais icônicos do terror."
    }}
}}"""
...
async def text_to_speech_coqui(text, filename, velocidade=1.0):
    """
    Converte texto em áudio usando Coqui TTS e ajusta a velocidade mantendo alta qualidade.
    """
    # Substitui pontos para melhor separação de sentenças
    text = text.replace(".", ";")

    # Gera o áudio com Coqui TTS
    tts = TTS("tts_models/multilingual/multi-dataset/xtts_v2")
    tts.tts_to_file(text=text, speaker="Luis Moray", language="pt", file_path=filename)

    if velocidade != 1.0:
        try:
            # Ajusta a velocidade usando Rubberband diretamente no arquivo
            temp_file = filename.replace(".mp3", f"_temp_{velocidade}.mp3")
            adjust_speed_rubberband(filename, temp_file, velocidade)

            #Remove o arquivo original e renomeia o temporário para o nome final
            if os.path.exists(filename):
                os.remove(filename)
            os.rename(temp_file, filename)
        except Exception as e:
            print(f"Erro ao ajustar a velocidade com Rubberband: {str(e)}")
    
    return os.path.abspath(filename)

def adjust_speed_rubberband(input_path, output_path, speed):
    """
    Ajusta a velocidade do áudio sem alterar o pitch, utilizando o Rubberband com configurações otimizadas para qualidade.
    
    Parâmetros:
        input_path (str): Caminho para o arquivo de entrada (áudio original).
        output_path (str): Caminho para o arquivo de saída (áudio ajustado).
        speed (float): Fator de ajuste de velocidade (e.g., 2.0 = 2x mais rápido).
    """
    # Comando do Rubberband com configurações de qualidade máxima
    command = [
        "rubberband",
        "-3",  # Motor R3 para maior qualidade
        "--formant",  # Preserva os formantes para manter o timbre natural
        "--window-long",  # Usa janela longa para melhor qualidade
        "--pitch-hq",
        "--tempo", str(speed),  # Ajusta a velocidade sem mudar o pitch
        "--quiet",  # Suprime mensagens desnecessárias
        input_path,
        output_path
    ]
...
async def process_file_async(file_path, start_time, output_dir, script_data=None, is_last_file=False, is_first_file=False):
    ...
    if text:
        await text_to_speech_coqui(text, audio_path, velocidade=args.voice_speed)
    ...
    if file_extension == '.mp4':
        ...
        if audio_path and os.path.exists(audio_path):
            ...
            # Se for o primeiro arquivo, adicionar 1 segundos de silêncio inicial apenas no áudio
            if is_first_file:
                print("Adicionando 1 segundos de silêncio inicial ao áudio")
                initial_silence = AudioClip(lambda t: 0, duration=1)
                initial_silence = initial_silence.set_fps(audio_clip.fps)
                audio_clip = concatenate_audioclips([initial_silence, audio_clip])
                audio_duration = audio_clip.duration  # Atualizar a duração do áudio
            
            # Se o vídeo for mais curto que o áudio, estender sua duração
            if video_duration < audio_duration:
                print(f"Estendendo vídeo de {video_duration}s para {audio_duration}s")
            ...
            if video_duration > audio_duration:
                print(f"Adicionando silêncio para cobrir diferença de {video_duration - audio_duration}s")
            ...
            # Se for o último arquivo, adicionar 3 segundos extras no final
            if is_last_file:
                print("Adicionando 3 segundos ao final do último clip")
                final_duration += 3
                # Estender o vídeo com o último frame
                last_frame = video_clip.to_ImageClip(t=video_clip.duration - 1)
                last_frame = last_frame.set_duration(3)
                video_clip = concatenate_videoclips([video_clip, last_frame], method="compose")
                
                # Adicionar 3 segundos de silêncio ao áudio
                extra_silence = AudioClip(lambda t: 0, duration=3)
                extra_silence = extra_silence.set_fps(final_audio.fps)
                final_audio = concatenate_audioclips([final_audio, extra_silence])
            ...
            print(f"Duração final do clip: {final_duration}s")
            return video_clip, audio_path, None, start_time + video_clip.duration
...
async def create_video_async(args, run_dir, script_data=None):
    ...
    data_files = [f for f in os.listdir(data_dir) if f.lower().endswith(('.mp4'))]
    ...
    for i, file in enumerate(data_files):
        try:
            ...
            # Verificar se é o primeiro ou último arquivo
            is_first_file = (i == 0)
            is_last_file = (i == len(data_files) - 1)
            
            clip, audio_path, subtitle, new_start_time = await process_file_async(
                file_path, 
                start_time, 
                run_dir, 
                script_data,
...
def convert_whisper_segments(segments: List[Dict], words_per_segment: int = 40, min_silence: float = 0.3) -> List[tuple]:
    """
...
async def main():
    ...
    if args.script_mode == 'temp_f':
        try:
           
            # Use temp directory from root instead of run directory
            root_temp_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'temp')
            
            # Get all script files from root temp directory
            script_files = [f for f in os.listdir(root_temp_dir) 
                           if f.startswith('script_') and f.endswith('.json')]
            
            # Process each script file
            for script_file in script_files:
                print(f"\nProcessando roteiro: {script_file}")
                script_path = os.path.join(root_temp_dir, script_file)
                ...
                # script_path = os.path.join('C://Users//Andre//Desktop//Projetos//auto_tube//run_16-11-2024-01_24_43//temp', 'script.json')
                # with open(script_path, 'r', encoding='utf-8') as f:
                #               script_data = json.load(f)
                # await create_video_async(args, 'C://Users//Andre//Desktop//Projetos//auto_tube//run_16-11-2024-01_24_43', script_data)
                ...
                if enable_sub:
                    ...
                    subtitled_video = await add_subtitles_to_video(
                        ...
                        whisper_model_size="large-v3",  # você pode ajustar o tamanho do modelo
