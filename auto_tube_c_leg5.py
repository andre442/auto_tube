...
def parse_arguments():
    ...
    parser.add_argument('--quality', type=float, default=1.0, help='qualidade das imagens (1 qualidade máxima, valores maiores diminuem a qualidade)')
    parser.add_argument('--paymode', type=str, default='default', help='modo payload SD')
    parser.add_argument('--v_type', type=str, default='default', help='tipo do roteiro a ser gerado (kids), (default)')
...
{{
    ...
    "seo": {{
        ...
        elif args.v_type == 'kids':
            system_prompt = f"""
Você é um especialista em criar histórias infantis mágicas e envolventes para crianças de 4 a 6 anos, com experiência em conteúdo viral do TikTok.
Crie um roteiro para um vídeo narrado em português de {duration} segundos que:
- Tenha um personagem principal carismático (preferencialmente um animal)
- Inclua uma lição simples e positiva
- Use linguagem adequada para crianças pequenas
- Tenha elementos de repetição ou padrões que as crianças possam acompanhar
- Inclua momentos de surpresa ou "plot twists" leves

Diretrizes técnicas:
- São necessárias {target_scenes} cenas com duração média de {avg_scene_duration:.1f} segundos
- Para as imagens:
  * Use cores vibrantes e estilo ilustração infantil fofo (cute children's illustration style)
  * Prefira cenas amplas e coloridas
  * Evite close-ups ou detalhes complexos
  * Mantenha um estilo visual consistente entre as cenas
  * Especifique sempre "cartoon for kids" ou "children's book illustration style" nos prompts
  * O image_prompt precisa ser em inglês
  * Não gere prompts para imagem com nomes próprios como "Pedro's playing with a ball at the beach". Ao invés disso, gere sempre algo como "A black dog playing with a ball at the beach" se o vídeo for sobre um cachorro.
- A narração deve:
  * Incluir repetições ou rimas ocasionais
  * Ter um ritmo adequado para TikTok
  * Manter tom alegre e acolhedor
        
Estrutura exata do JSON esperado:
{{
    "scenes": [
        {{
            "image_prompt": "Prompt em inglês para o Stable Diffusion que represente a cena.",
            "narration": "Texto que deve ser narrado durante esta cena, composto por 2 ou 3 frases.",
            "duration": 10
        }},
        {{
            "image_prompt": "Prompt em inglês para a imagem da cena 2",
...
async def process_file_async(file_path, start_time, output_dir, script_data=None, is_last_file=False, is_first_file=False):
    ...
    if file_extension == '.mp4':
        ...
        if audio_path and os.path.exists(audio_path):
            ...
            # Se for o primeiro arquivo, adicionar 3 segundos de silêncio inicial apenas no áudio
            if is_first_file:
                print("Adicionando 3 segundos de silêncio inicial ao áudio")
                initial_silence = AudioClip(lambda t: 0, duration=3)
...
async def add_subtitles_to_video(
    ...
    try:
        ...
        result = model.transcribe(
            ...
            hallucination_silence_threshold=0.9
        )
...
async def main():
    try:
          ###check_gpu_availability_windows()
